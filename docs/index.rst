...
1.  PyDataColl use `Redis <http://redis.io/download>`_ to store collected data and need
    `Python <https://www.python.org/downloads/>`_ >=3.5, make sure you have installed them.(if you're under Windows,
    use `Redis for win <https://github.com/MSOpenTech/redis/releases/>`_)

2.  Download the latest source code, open a shell and go to the source directory, install required package using pip::

        pip install -r requirements

3.  Open a shell(*CMD* in Windows, *Terminal* in Linux and Mac OS X), run the following to start a DeviceManager first::

        python -m pydatacoll.plugins.device_manage

4.  Then open another shell and run this to start the server::

        python -m pydatacoll.api_server

.. note::
    to stop the server, press CTRL+C to exit

5.  Congratulations! The server is running now. Visit http://localhost:8080 in browser to see the server information, if
...
6.  Now you can send request to server with your favorite http client! check :doc:`restapi` to see the API list.
