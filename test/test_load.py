...
""")
    ...
    async def test_heavy_load(self):
        ...
        async with aiohttp.ClientSession() as ss:
            async with ss.post('http://127.0.0.1:8080/api/v2/devices', json=device_list) as r:
                self.assertEqual(r.status, 200)
            logger.info('create term..')
            async with ss.post('http://127.0.0.1:8080/api/v2/terms', json=term_list) as r:
                self.assertEqual(r.status, 200)
            logger.info('create item..')
            async with ss.post('http://127.0.0.1:8080/api/v2/items', json=item_list) as r:
                self.assertEqual(r.status, 200)
            logger.info('create formula..')
            async with ss.post('http://127.0.0.1:8080/api/v2/formulas', json=formula_list) as r:
                self.assertEqual(r.status, 200)
            logger.info('create term_item..')
            async with ss.post('http://127.0.0.1:8080/api/v2/term_items', json=term_item_list) as r:
                self.assertEqual(r.status, 200)
        logger.info('done.')
