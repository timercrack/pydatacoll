...
CP56Time2a = ExprAdapter(
    ...
    encoder=lambda time, ctx: Container(Year=time.year % 2000, Month=time.month, Day=time.day, Week=time.isoweekday(),
                                        Hour=time.hour, SU=0, Minute=time.minute, IV=0,
                                        Millisecond=time.microsecond // 1000 + time.second * 1000),
    decoder=lambda obj, ctx: datetime.datetime(year=obj.Year + 2000, month=obj.Month, day=obj.Day,
                                               hour=obj.Hour, minute=obj.Minute, second=obj.Millisecond // 1000,
...
ASDU_C_CI_NA_1 = Struct(
    ...
    EmbeddedBitStruct(BitField("Address", 24, swapped=True)),  # 信息对象地址
    EmbeddedBitStruct(
        Bits("FRZ", 2),  # 0 读（无冻结复位） 1 计数量冻结不带复位（累加值） 2 冻结带复位（增量信息） 3 计数量复位
...
def init_frame(cls, apci1=None, apci2=None, TYP=None, cause=Cause.unused, SQ_COUNT=1, SQ=0):
    cc = Container(APCI1=apci1, APCI2=apci2, length=0, ASDU=None)
    if TYP is not None:
        cc.ASDU = Container(TYP=TYP, SQ=SQ, SQ_COUNT=SQ_COUNT, T=0, PN=0, SourceAddress=0,
                            StartAddress=0, Cause=cause, GlobalAddress=1, data=list())
