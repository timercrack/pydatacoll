...
ASDU_Part = Struct(
        ...
        ExprAdapter(
                ULInt16('Cause'),
                encoder=lambda obj, ctx: (obj.SourceAddress << 8) + (obj.T << 7) + (obj.PN << 6) + obj.Cause
                if isinstance(obj, Container) else obj,
...
class IEC104Adapter(Subconstruct):
    __slots__ = []

    def _parse(self, stream, context):
        frame = self.subcon._parse(stream, context)
        return self._decode(self.subcon._parse(stream, context), context)

    def _build(self, obj, stream, context):
        build = self.subcon._build(obj, stream, context)
