...
logger = my_logger.get_logger('GB31960Device')
...
class GB31960Device:
    ...
    def save_frame(self, frame, send=True):
        GB31960Device.frame_list[self.device_id].append(('send' if send else 'recv', frame))
    ...
    for device_id in device_list:
        ...
        server_list.append(
            io_loop.run_until_complete(
                    asyncio.start_server(functools.partial(GB31960Device, device), '127.0.0.1', device['port'])))
    return server_list
